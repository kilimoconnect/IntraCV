...
export async function downloadCvAsPdf(
  ...
  `;
  /* zoom:1 !important above overrides ALL inline zoom values on descendant
     elements (CSS !important in author stylesheets beats inline styles).
     stabiliseFontMetrics then reads the original inline zoom value via
     el.style.zoom (unaffected by !important) and applies transform:scale
     for elements that need to shrink (zoom < 1 path). */
