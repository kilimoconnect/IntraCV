...
function UpgradeContent() {
  ...
  const handlePay = async () => {
    ...
    setLoading(true);
    try {
      const redirectUrl = `${window.location.origin}/interview-payment/flutterwave-callback?action=${pendingAction}`;
  ...
  const PayButton = ({ className = "" }: { className?: string }) => (
    <Button
      onClick={handlePay}
      disabled={loading}
      className={`bg-[#f5a623] hover:bg-[#e09510] text-white border-0 font-bold shadow-lg disabled:opacity-60 ${className}`}
    >
      {loading
