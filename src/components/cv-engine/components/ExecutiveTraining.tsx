...
export default function ExecutiveTraining({ data, style, widthPx, heightPx, isSidebar, sectionTitle, renderSettings: rs }: Props) {
  const items = data.executiveTraining || [];
  ...
  return (
    <div style={{
      ...
      <SectionHeading title={sectionTitle || "Executive Training"} style={style} isSidebar={isSidebar} renderSettings={rs} />
      {items.map((t, i) => {
          ...
          <div key={i} style={{ display: "flex", gap: "7px", marginBottom: "5px", alignItems: "flex-start" }}>
            ...
            <div>
              ...
              <div style={{ fontSize: sf(8, rs), color: muted, letterSpacing: "0.2px" }}>
                {[t.institution, t.year].filter(Boolean).join(" · ")}
              </div>
            </div>
          </div>
        );
